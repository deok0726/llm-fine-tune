...
def chunk_text(text, max_tokens, tokenizer):
    ...
    for i in range(0, input_ids.shape[-1], max_tokens):
...
def summarize_news(path):
    with open("/svc/project/genaipilot/fine-tune/output/news_8b_kor.txt", 'w') as f:
        ...
        for text in sorted_list:
            ...
            prompt = '''롯데카드는 대한민국의 신용카드 회사이며, 로카는 롯데카드의 줄임말이고, LOCA는 롯데카드의 약자입니다. 당신은 롯데카드의 온라인 뉴스 관리 직원입니다. 당신의 임무는 인터넷에서 뉴스 기사를 찾아 요약하고 분석하는 것입니다. 업로드되는 txt 파일에는 뉴스 기사의 링크, 제목, 출처, 날짜, 키워드, 내용이 포함되어 있습니다.'''
            instruction = f'''기사 요약 생성
...
def summarize_sns(path):
    with open("/svc/project/genaipilot/fine-tune/output/sns_8b_kor.txt", 'w') as f:
        file_list = os.listdir(path)
        sorted_list = sorted(file_list, key=lambda x: int(x.split('.')[0]))
        for text in sorted_list:
            print(text)
            file_path = os.path.join(path, text)
            document_text = read_text_file(file_path)
            prompt = '''롯데카드는 대한민국의 신용카드 회사이며, 로카는 롯데카드의 줄임말이고, LOCA는 롯데카드의 약자입니다. 당신은 신용카드 회사인 롯데카드의 평판 관리 직원입니다. 여러분의 임무는 롯데카드에 대한 뉴스와 소셜 미디어 평판을 수집하여 요약과 인사이트를 얻는 것입니다. 업로드되는 txt 파일에는 커뮤니티 게시물의 키워드, 링크, 제목, 작성일, 추천 수, 댓글 수, 출처, 본문, 반응이 포함되어 있습니다.'''
            instruction = f'''게시글 요약 생성
...
def insight_news(text):
    with open("/svc/project/genaipilot/fine-tune/output/news_8b_insight_kor.txt", 'w') as f:
        document_text = read_text_file(text)
        prompt = '''롯데카드는 대한민국의 신용카드 회사이며, 로카는 롯데카드의 줄임말이고, LOCA는 롯데카드의 약자입니다. 여러분은 롯데카드의 온라인 뉴스 관리 직원입니다. 여러분의 임무는 인터넷에서 뉴스 기사를 찾아 요약하고 분석하는 것입니다. 업로드되는 txt 파일에는 뉴스 기사의 제목, 날짜, 출처, 키워드, 요약, 링크가 포함되어 있습니다.'''
        instruction = f'''
                        분석 인사이트 생성
        ...
        max_tokens = model.config.max_position_embeddings - 100  # Reserve some space for generated tokens
        chunks = chunk_text(input_text, max_tokens, tokenizer)
        ...
        for chunk in chunks:
            outputs = model.generate(
                ...
                max_new_tokens=1024,  # Adjust this if necessary
        ...
        # generated_tokens = outputs[:, max_prompt_length:]
...
def insight_sns(text):
    with open("/svc/project/genaipilot/fine-tune/output/sns_8b_insight_kor.txt", 'w') as f:
        document_text = read_text_file(text)
        prompt = '''롯데카드는 대한민국의 신용카드 회사이며, 로카는 롯데카드의 줄임말이고, LOCA는 롯데카드의 약자입니다. 당신은 신용카드 회사인 롯데카드의 평판 관리 직원입니다. 여러분의 임무는 롯데카드에 대한 뉴스와 소셜 미디어 평판을 수집하여 요약과 인사이트를 얻는 것입니다. 업로드되는 txt 파일에는 커뮤니티 게시물의 링크, 제목, 작성 날짜, 출처, 요약, 반응, 성향이 포함되어 있습니다.'''
        instruction = f'''
                        시사점 도출
...
if __name__ == "__main__":
    ...
    insight_news("/svc/project/genaipilot/fine-tune/output/news_8b_kor.txt")
    insight_sns("/svc/project/genaipilot/fine-tune/output/sns_8b_kor.txt")