...
def compare_contract(client, a, b, c):
...
def renew_contract(client, old, new):
    with open("/svc/project/llm-fine-tune/renew_3.txt", 'w') as f:
...
def customer_call(client, path):
    with open("/svc/project/llm-fine-tune/true.txt", 'w') as f:
...
def summarize_news(client, path):
    with open("/svc/project/llm-fine-tune/output/summary_news_claude.txt", 'w') as f:
        file_list = os.listdir(path)
...
def summarize_sns(client, path):
    with open("/svc/project/llm-fine-tune/output/summary_sns_claude.txt", 'w') as f:
        file_list = os.listdir(path)
        sorted_list = sorted(file_list, key=lambda x: int(x.split('.')[0]))
        for text in sorted_list:
            print(text)
...
def insight_news(client, text):
    with open("/svc/project/llm-fine-tune/output/insight_news_claude.txt", 'w') as f:
...
def insight_sns(client, text):
    with open("/svc/project/llm-fine-tune/output/insight_sns_claude.txt", 'w') as f:
        document_text = read_text_file(text)
...
def summarize_promotion(client, path):
    with open("/svc/project/llm-fine-tune/output/promotion_sep_claude.txt", 'w') as f:
...
def insight_promotion(client, text):
    document_text = read_text_file(text)
    with open("/svc/project/llm-fine-tune/output/insight_sep_claude.txt", 'w') as f:
...
def compare_promotion(client, text_a, text_b):
    document_a = read_text_file(text_a)
    document_b = read_text_file(text_b)
    with open("/svc/project/llm-fine-tune/output/compare_promotion_claude.txt", 'w') as f:
        system_instruction = '당신은 카드사의 캐시백 혜택 요약으로부터 질문에 해당하는 답변을 제공합니다. 요약 내용에 기반하여 시사점을 도출할 수 있는 답변을 생성해주세요'
        user_prompt = f'''
...
if __name__ == "__main__":
    ...
    # summarize_promotion(client, "/svc/project/llm-fine-tune/data/sep")
    # insight_promotion(client, "/svc/project/llm-fine-tune/output/promotion_sep_claude.txt")
    compare_promotion(client, "/svc/project/llm-fine-tune/output/insight_sep_claude.txt", "/svc/project/llm-fine-tune/output/insight_oct_claude.txt")

    # summarize_news(client, "/svc/project/llm-fine-tune/data/news")
    # summarize_sns(client, "/svc/project/llm-fine-tune/data/sns")
    # insight_news(client, "/svc/project/llm-fine-tune/output/summary_news_claude.txt")
    # insight_sns(client, "/svc/project/llm-fine-tune/output/summary_sns_claude.txt")

    # customer_call(client, "/svc/project/llm-fine-tune/data/call/true")

    # old = "/svc/project/llm-fine-tune/data/contract/(3) 채권추심 위임 계약서/③ 23년 4분기 채권추심 위임 계약_1.표준위탁계약서_붙임.docx"
    # new = "/svc/project/llm-fine-tune/data/contract/(3) 채권추심 위임 계약서/③ 24년 1분기 채권추심 위임 계약_1.표준위탁계약서_붙임.docx"
    # renew_contract(client, old, new)
