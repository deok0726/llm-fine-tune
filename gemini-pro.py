...
def summary_contract(path):
    ...
    system_instruction = '''당신의 역할은 계약서의 핵심 내용을 파악하고, 이를 요약하는 것입니다. 아래 지시사항을 따라서 계약서 요약을 수행합니다.
                            ...
                            Think step by step. 
                            Answer in Korean. '''
    user_prompt = f'''  업무위탁계약서: {document_text}
                        요약: '''
...
def customer_call(path):
    with open("/svc/project/llm-fine-tune/output/false_gemini.txt", 'w') as f:
...
def summarize_news(path):
    with open("/svc/project/llm-fine-tune/output/summary_news_gemini.txt", 'w') as f:
        file_list = os.listdir(path)
        ...
        for text in sorted_list:
            ...
            if response.prompt_feedback:
                f.write(text + "\n")
                f.write(f"차단 사유: {response.prompt_feedback.block_reason}")
                f.write("\n" + "-"*100 + "\n")
            else:
                print(response.text)
                f.write(text + "\n")
...
def summarize_sns(path):
    with open("/svc/project/llm-fine-tune/output/summary_sns_gemini.txt", 'w') as f:
        file_list = os.listdir(path)
        sorted_list = sorted(file_list, key=lambda x: int(x.split('.')[0]))
        for text in sorted_list:
            print(text)
...
def insight_news(text):
    with open("/svc/project/llm-fine-tune/output/insight_news_gemini.txt", 'w') as f:
...
def insight_sns(text):
    with open("/svc/project/llm-fine-tune/output/insight_sns_gemini.txt", 'w') as f:
        document_text = read_text_file(text)
...
def summarize_promotion(path):
    with open("/svc/project/llm-fine-tune/output/promotion_oct_gemini.txt", 'w') as f:
...
def insight_promotion(text):
    document_text = read_text_file(text)
    with open("/svc/project/llm-fine-tune/output/insight_oct_gemini.txt", 'w') as f:
...
def compare_promotion(text_a, text_b):
    document_a = read_text_file(text_a)
    document_b = read_text_file(text_b)
    with open("/svc/project/llm-fine-tune/output/compare_promotion_gemini.txt", 'w') as f:
        system_instruction = '''당신은 카드사의 캐시백 혜택 요약으로부터 질문에 해당하는 답변을 제공합니다. 요약 내용에 기반하여 시사점을 도출할 수 있는 답변을 생성해주세요.
...
if __name__ == "__main__":
    # summary_contract("/svc/project/llm-fine-tune/data/contract/01_업무위탁계약서(표준)2024 수정_비식별화.docx")
    ...
    # compare_contract(a, b, c)

    # customer_call("/svc/project/llm-fine-tune/data/call/false")
    ...
    summarize_news("/svc/project/llm-fine-tune/data/news")
    summarize_sns("/svc/project/llm-fine-tune/data/sns")

    insight_news("/svc/project/llm-fine-tune/output/summary_news_gemini.txt")
    insight_sns("/svc/project/llm-fine-tune/output/summary_sns_gemini.txt")

    # summarize_promotion("/svc/project/llm-fine-tune/data/oct")
    # insight_promotion("/svc/project/llm-fine-tune/output/promotion_oct_gemini.txt")
    # compare_promotion("/svc/project/llm-fine-tune/output/promotion_sep_gemini.txt", "/svc/project/llm-fine-tune/output/promotion_oct_gemini.txt")